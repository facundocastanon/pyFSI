...
class nlLFB1D(fsiBase):
    ...
    def __init__(self, execution, control, TIME, OREG):
        ...
        # ----- Public attributes ----- #
        self.dof = 2 * self._solid.dof + self._flow.dof  # DOFs of the FSI system
        self.state = None  # Current State
        self.fState = None # Fluid State
        self.sState = None # Solid State
        self.rhs = np.zeros(self.dof)  # Current variable velocities

        # ----- Private attributes ----- #
        # Indexing coefficients
        self._i0 = 0
        self._i1 = self._solid.dof
        self._i2 = 2 * self._solid.dof
        self._i3 = self.dof
    ...
    def setInitialConditions(self, t0):
        solid = self._solid
        flow = self._flow
        solid.setInitialConditions(t0)
        #flow.setInitialConditions(t0)
    ...
    def rhsSolid(self, time, solidState):
        ...
        # printArray(solid.F, "F")
        # ff = solid.addedStateModalForce(flow.deltaPx[0])
        # printArray(ff, "ff")
