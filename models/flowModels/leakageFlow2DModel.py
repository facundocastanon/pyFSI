...
class leakageFlow2D(flowModel):
    ...
    def __init__(self, execution, control, mesh, boundary, time, name='NN'):
        ...
        self.Eq = None
        self.Gq = None
        self.s = None #Channel size distribution
        # Output variable mapping
        self.varMap["flowRates"]  = "Q0"
        self.varMap["flowSpeeds"] = "vRef"
        self.varMap["channelSize"] = "s"
        ...
        self._eigen = self._mesh.getObjectRegistry().get(control['eigen']).eigen
        self._size = self._eigen.size  # Size of the associated eigensystem

        # ----- Procedures ----- #
        # Initialize the boundary conditions
        # Flow Rate BC
        Q0BCDict = self.control['bc']['inlet']['Q0']
        self._Q0InBC = getattr(boundaryConditions, Q0BCDict['type'])(Q0BCDict)
        Q0BCDict = self.control['bc']['outlet']['Q0']
    ...
    def _m(self):
        ...
        for region in self.regions:  # Loop for filling the mass matrix
            ...
            for i in range(0, self._size):
                m[i] += self.integrate(self._eigen.ix[i] / r['he'], region)
    ...
    def _c(self):
        ...
        for region in self.regions:  # Loop for filling the damping matrix
            ...
            for i in range(self._size):
                ...
                tvf = - gix / r['he3']
                cvf[i] += ((0.5 * self._f0 + 0.25 * self.Q0 * self._eta) * self.integrate(tvf, region))
                # Inlet-outlet damping
                cio[i] += ((self._zetaOutBC.getValue(self._time.value) / r['he2'][-1]) * 
                           self._eigen.iL[i] * r['hexL'])
    ...
    def _Bq(self):
        ...
        for region in self.regions:  # Loop for filling the damping matrix
            ...
            for i in range(self._size):
                Bq[region.name] += - (self._th * si.simps(self._eigen.ix[i] /
    ...
    def _Dq(self):
        ...
        for region in self.regions: # Loop for filling the damping matrix
            ...
            for i in range(self._size):
                # Nonlinear term
                dnl += (2 * self._xix * si.simps(self._eigen.ix[i] * r['hed'] / r['he3']
                        - self._eigen.vectors[i] / r['he2'], self._mesh.x))
                # Friction term
                dvf += -((0.5 * self._f0 + 0.25 * self.Q0 * self._eta) * 
                         si.simps(self._eigen.ix[i] / r['he3'], self._mesh.x))
                # Boundary Terms
                dio += -self._zetaOutBC.getValue(self._time.value) / r['he2'][-1] * self._eigen.iL[i]
    ...
    def integrate(self, term, region):
        ...
        x = self._mesh.x
        result = si.cumtrapz(term, x, initial=0.0) - si.simps(term, x) * region.data['hexL']
